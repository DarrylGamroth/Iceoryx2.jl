...
| --- | --- | --- | --- |
| ABI-001 | Bind exactly `Iceoryx2_jll v0.9.999+2` | exact compat pins in root/gen projects; `gen/gen-bindings.jl` | `scripts/coverage_symbols.jl` reports 682/682; `gen/verify_generated.jl` performs a non-destructive byte comparison |
...
| API-005 | Expose progressive buffer ownership and atomic progress | `announce!`, `unsafe_commit_until!`, `complete!`, `abort!`, and coherent snapshots in `src/messaging/progressive_publish_subscribe.jl` | `test/progressive_pubsub.jl` for both transports, including zero-allocation gates |
