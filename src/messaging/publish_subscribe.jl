...
"""
function user_header(builder::PubSubServiceBuilder{S, T, Nothing}, ::Type{UH}) where {
        S, T, UH}
    _require_valid(builder.handle, "publish_subscribe service builder")
...
function service_hash(factory::PortFactoryPubSub)
    ...
    return _service_hash_string(
        ...
        Iceoryx2FFI.iox2_port_factory_pub_sub_service_hash
    )
end

function try_cleanup_dead_nodes(factory::PortFactoryPubSub)
    _require_valid(factory.handle, "publish_subscribe port factory")
    state = _cleanup_state_ref()
    Iceoryx2FFI.iox2_port_factory_pub_sub_try_cleanup_dead_nodes(
        Ref{Iceoryx2FFI.iox2_port_factory_pub_sub_h}(factory.handle),
        state
    )
...
function blocking_cleanup_dead_nodes(factory::PortFactoryPubSub, seconds::Integer, nanoseconds::Integer = 0)
    _require_valid(factory.handle, "publish_subscribe port factory")
    secs, nanos = _timeout_parts(seconds, nanoseconds)
    state = _cleanup_state_ref()
    Iceoryx2FFI.iox2_port_factory_pub_sub_blocking_cleanup_dead_nodes(
        Ref{Iceoryx2FFI.iox2_port_factory_pub_sub_h}(factory.handle),
        state,
        secs,
        nanos
...
function blocking_cleanup_dead_nodes(factory::PortFactoryPubSub, seconds::Real)
...
function open_or_create(builder::PubSubServiceBuilder{S, T, UH}, verifier::AttributeVerifier) where {
        S, T, UH}
    _require_valid(builder.handle, "publish_subscribe service builder")
...
function open(builder::PubSubServiceBuilder{S, T, UH}, verifier::AttributeVerifier) where {
        S, T, UH}
    _require_valid(builder.handle, "publish_subscribe service builder")
    _require_valid(unsafe_handle(verifier), "attribute verifier")
    storage = Ref{Iceoryx2FFI.iox2_port_factory_pub_sub_t}()
...
function create(builder::PubSubServiceBuilder{S, T, UH}, specifier::AttributeSpecifier) where {
        S, T, UH}
    _require_valid(builder.handle, "publish_subscribe service builder")
...
function open(f::Function, builder::PubSubServiceBuilder{S, T, UH},
        verifier::AttributeVerifier) where {S, T, UH}
...
function create(f::Function, builder::PubSubServiceBuilder{S, T, UH},
        specifier::AttributeSpecifier) where {S, T, UH}
...
function open_or_create_with_attributes(builder::PubSubServiceBuilder{S, T, UH},
        verifier::AttributeVerifier) where {S, T, UH}
    open_or_create(builder, verifier)
end
function open_or_create_with_attributes(
        f::Function, builder::PubSubServiceBuilder{S, T, UH},
        verifier::AttributeVerifier) where {S, T, UH}
    open_or_create(f, builder, verifier)
end
function open_with_attributes(builder::PubSubServiceBuilder{S, T, UH},
        verifier::AttributeVerifier) where {S, T, UH}
    open(builder, verifier)
end
function open_with_attributes(f::Function, builder::PubSubServiceBuilder{S, T, UH},
        verifier::AttributeVerifier) where {S, T, UH}
    open(f, builder, verifier)
end
function create_with_attributes(builder::PubSubServiceBuilder{S, T, UH},
        specifier::AttributeSpecifier) where {S, T, UH}
    create(builder, specifier)
end
function create_with_attributes(f::Function, builder::PubSubServiceBuilder{S, T, UH},
        specifier::AttributeSpecifier) where {S, T, UH}
...
mutable struct PublisherBuilder{S, T, UH}
    ...
    function PublisherBuilder{S, T, UH}(handle, storage, keepalive) where {S, T, UH}
        obj = new{S, T, UH}(handle, storage, keepalive, Any[])
        finalizer(Base.close, obj)
        return obj
    end
end
...
function publisher_builder(factory::PortFactoryPubSub{S, T, UH}) where {S, T, UH}
    ...
    handle = Iceoryx2FFI.iox2_port_factory_pub_sub_publisher_builder(
        Ref{Iceoryx2FFI.iox2_port_factory_pub_sub_h}(factory.handle), storage)
...
function name!(
    ...
    return _set_port_name!(
        builder,
        name,
        Iceoryx2FFI.iox2_port_factory_publisher_builder_set_name,
...
function allocation_strategy!(builder::PublisherBuilder, value::Union{
...
function backpressure_strategy!(
    ...
    _require_valid(builder.handle, "publisher builder")
    Iceoryx2FFI.iox2_port_factory_publisher_builder_backpressure_strategy(
        Ref{Iceoryx2FFI.iox2_port_factory_publisher_builder_h}(builder.handle),
...
function backpressure_handler!(builder::PublisherBuilder, handler::AbstractBackpressureHandler)
    _require_valid(builder.handle, "publisher builder")
    Iceoryx2FFI.iox2_port_factory_publisher_builder_set_backpressure_handler(
        Ref{Iceoryx2FFI.iox2_port_factory_publisher_builder_h}(builder.handle),
        handler.callback,
        handler.ref
    )
...
function backpressure_handler!(builder::PublisherBuilder, on_backpressure)
    backpressure_handler!(builder, BackpressureHandler(on_backpressure))
end

function degradation_handler!(builder::PublisherBuilder, handler::AbstractDegradationHandler)
    _require_valid(builder.handle, "publisher builder")
    Iceoryx2FFI.iox2_port_factory_publisher_builder_set_degradation_handler(
        Ref{Iceoryx2FFI.iox2_port_factory_publisher_builder_h}(builder.handle),
        handler.callback,
        handler.ref
    )
...
function degradation_handler!(builder::PublisherBuilder, on_degradation)
    degradation_handler!(builder, DegradationHandler(on_degradation))
end
...
@inline Base.isvalid(builder::Union{
    PublisherBuilder, SubscriberBuilder}) = builder.handle != _IOX2_NULL
...
function subscriber_builder(factory::PortFactoryPubSub{S, T, UH}) where {S, T, UH}
    ...
    handle = Iceoryx2FFI.iox2_port_factory_pub_sub_subscriber_builder(
        Ref{Iceoryx2FFI.iox2_port_factory_pub_sub_h}(factory.handle), storage)
...
function name!(
    ...
    return _set_port_name!(
        builder,
        name,
        Iceoryx2FFI.iox2_port_factory_subscriber_builder_set_name,
        Ref{Iceoryx2FFI.iox2_port_factory_subscriber_builder_h}(builder.handle),
        "subscriber builder"
    )
end

function buffer_size!(builder::SubscriberBuilder, value::Integer)
    _require_valid(builder.handle, "subscriber builder")
    Iceoryx2FFI.iox2_port_factory_subscriber_builder_set_buffer_size(
        Ref{Iceoryx2FFI.iox2_port_factory_subscriber_builder_h}(builder.handle),
...
function degradation_handler!(builder::SubscriberBuilder, handler::AbstractDegradationHandler)
    _require_valid(builder.handle, "subscriber builder")
    Iceoryx2FFI.iox2_port_factory_subscriber_builder_set_degradation_handler(
        Ref{Iceoryx2FFI.iox2_port_factory_subscriber_builder_h}(builder.handle),
        handler.callback,
        handler.ref
    )
...
function degradation_handler!(builder::SubscriberBuilder, on_degradation)
    degradation_handler!(builder, DegradationHandler(on_degradation))
end
...
mutable struct Publisher{S, T, UH}
    ...
    keepalive::PortFactoryPubSub{S, T, UH}
    callback_keepalive::Vector{Any}
    function Publisher{S, T, UH}(handle, storage, keepalive, callback_keepalive) where {
            S, T, UH}
        obj = new{S, T, UH}(handle, storage, keepalive, callback_keepalive)
        finalizer(Base.close, obj)
        return obj
    end
end
...
function Base.close(pub::Publisher)
    ...
    pub.storage = nothing
    _drop_callback_keepalive!(pub)
    return nothing
end

@inline function backpressure_strategy(pub::Publisher)
    _require_valid(pub.handle, "publisher")
    return _backpressure_strategy_enum(
        Iceoryx2FFI.iox2_publisher_backpressure_strategy(Ref{Iceoryx2FFI.iox2_publisher_h}(pub.handle)),
    )
end
...
function create(builder::PublisherBuilder{S, T, UH}) where {S, T, UH}
    ...
    return Publisher{S, T, UH}(handle_ref[], storage, builder.keepalive, callbacks)
end
...
mutable struct Subscriber{S, T, UH}
    ...
    keepalive::PortFactoryPubSub{S, T, UH}
    callback_keepalive::Vector{Any}
    function Subscriber{S, T, UH}(handle, storage, keepalive, callback_keepalive) where {
            S, T, UH}
        obj = new{S, T, UH}(handle, storage, keepalive, callback_keepalive)
        finalizer(Base.close, obj)
        return obj
...
function create(builder::SubscriberBuilder{S, T, UH}) where {S, T, UH}
    ...
    return Subscriber{S, T, UH}(handle_ref[], storage, builder.keepalive, callbacks)
end
...
@inline function payload_number_of_bytes(sample::Sample)
    _require_valid(sample.handle_ref[], "sample")
    return Int(Iceoryx2FFI.iox2_sample_payload_number_of_bytes(sample.handle_ref))
end
...
@inline function header(sample::Sample)
    ...
    slot.handle_ref[] != _IOX2_NULL ||
        throw(ErrorException("failed to acquire sample header"))
...
@inline function payload_number_of_bytes(sample::SampleMut)
    _require_valid(sample.handle_ref[], "sample")
    return Int(Iceoryx2FFI.iox2_sample_mut_payload_number_of_bytes(sample.handle_ref))
end
...
@inline _pubsub_header_ref(header::PublishSubscribeHeader) = Ref{Iceoryx2FFI.iox2_publish_subscribe_header_h}(unsafe_handle(header))
@inline _pubsub_header_ref(header::PublishSubscribeHeaderRef) = unsafe_handle(header)

@inline function publisher_id(header::Union{
        PublishSubscribeHeader, PublishSubscribeHeaderRef})
    _require_valid(unsafe_handle(header), "publish/subscribe header")
...
@inline function number_of_elements(header::Union{
        PublishSubscribeHeader, PublishSubscribeHeaderRef})
    _require_valid(unsafe_handle(header), "publish/subscribe header")
    return Int(
...
"""
function loan_slice_uninit!(publisher::Publisher{S, T, UH},
        sample::SampleMut{T, UH}, n::Integer) where {S, T, UH}
...
"""
function try_loan_slice_uninit!(publisher::Publisher{S, T, UH},
        sample::SampleMut{T, UH}, n::Integer) where {S, T, UH}
    _require_valid(publisher.handle, "publisher")
    _require_inactive(sample, "sample")
    sample.handle_ref[] = _IOX2_NULL
...
"""
@inline function loan_uninit!(publisher::Publisher{S, T, UH}, sample::SampleMut{
        T, UH}) where {S, T, UH}
...
"""
@inline function try_loan_uninit!(publisher::Publisher{S, T, UH}, sample::SampleMut{
        T, UH}) where {S, T, UH}
...
"""
function loan_slice!(
        publisher::Publisher{S, T, UH}, sample::SampleMut{
            T, UH}, n::Integer) where {S, T, UH}
    loan_slice_uninit!(publisher, sample, n)
    fill!(payload_mut(sample), _default_value(T))
    return sample
end

function loan_slice!(f::Function, publisher::Publisher{S, T, UH},
        sample::SampleMut{T, UH}, n::Integer) where {S, T, UH}
    loan_slice!(publisher, sample, n)
    try
        return f(sample)
...
function loan_slice_uninit!(f::Function, publisher::Publisher{S, T, UH},
        sample::SampleMut{T, UH}, n::Integer) where {S, T, UH}
    loan_slice_uninit!(publisher, sample, n)
    try
        return f(sample)
...
"""
function send_copy(publisher::Publisher{S, T, UH}, data::Ptr{T}, n::Integer) where {
        S, T, UH}
    _require_valid(publisher.handle, "publisher")
    recipients = Ref{Iceoryx2FFI.c_size_t}()
    ret = Iceoryx2FFI.iox2_publisher_send_slice_copy(
        Ref{Iceoryx2FFI.iox2_publisher_h}(publisher.handle), data,
        Iceoryx2FFI.c_size_t(sizeof(T)), Iceoryx2FFI.c_size_t(n), recipients)
    check_ok(ret, Iceoryx2FFI.iox2_send_error_e)
...
"""
function receive!(f::Function, subscriber::Subscriber{S, T, UH}, sample::Sample{
        T, UH}) where {S, T, UH}
