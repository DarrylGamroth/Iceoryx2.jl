@testset "ServiceListing" begin
    ...
    names = String[]
    Iceoryx2.list_services(service_type=Iceoryx2.ServiceType.IPC) do cfg
        push!(names, Iceoryx2.name(cfg))
        return :continue
    end
    @test any(==(service_name), names)

    @test Iceoryx2.service_does_exist(service_name; service_type=Iceoryx2.ServiceType.IPC, messaging_pattern=:publish_subscribe)
